...
- 解析 TXT、Markdown、Python 代码和文本型 PDF，保留文件名、页码、行号、代码符号等来源定位。
...
明确不属于 Python 服务的职责：公网 HTTP API、认证与租户入口、会话、Chat Model 回答、Agent Loop、Tool Calling、MCP 和 SSE。这些能力将在未来的 Go 控制面实现，Go 仍通过同一份 protobuf 调用 Python。
...
摄取不是“上传后直接发消息”：原始字节先进入 staging；MySQL 在一个事务中创建 Document、Job、Task 和 `WAITING_OBJECT` Outbox；Finalizer 提升正式对象后才允许 Relay 发布 `task_id`。Worker 总是回读 MySQL 事实状态，成功写入新 ES 版本并通过条件检查后才切换可见版本和 ACK。

检索时，服务生成查询向量并向 ES 分别请求 Dense 与 BM25 候选，再用 MySQL 复核文档删除状态和 active version，最后执行 RRF、可选 Rerank、token 预算与 provenance 规范化。Python 返回 evidence，不生成最终回答或 `[n]` Citation 编号。
...
启动入口会校验 Compose、构建镜像、执行数据库迁移，并等待 MySQL、Elasticsearch、NATS、gRPC Server、Worker 与 Outbox 健康。gRPC 开发端点为 `localhost:50051`。
...
所有公共命令先进入 Makefile，再由 Earthly 固定 Python、uv 和依赖环境：
...
本项目采用 [Apache License 2.0](LICENSE)。设计借鉴 RAGFlow 等开源 RAG 系统的架构思想，不复制其产品规模或双语言重叠实现。
