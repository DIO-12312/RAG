...
| 同上 | `test_search_guard_operator_docs_preserve_private_tls_runbook` | Linux/Windows 生产 runbook 强制外部材料、snapshot、shard allocation、回滚与 fail-closed，禁止复用 development/test 材料服务或删卷；测试指南只使用公共 Make 真实验收入口并区分历史未受保护证据。 |
