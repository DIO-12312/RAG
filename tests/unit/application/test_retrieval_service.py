...
@pytest.mark.asyncio
async def test_chm_retrieve_expands_section_siblings_and_parent_context() -> None:
    now = datetime.now(UTC)
    repository = FakeMetadataRepository()
    model = FakeModelGateway(8)
    search = FakeSearchEngine()
    ...
    vector = (await model.embed(["exact API query"]))[0]
    base = {
    ...
    before = _chunk(
        ...
        ordinal=2,
        topic_path="topic-a.html",
        metadata_extra={**base, "chunk_index_in_section": "0"},
    )
    anchor = _chunk(
        "document-1",
        1,
        ...
        ordinal=3,
        topic_path="topic-a.html",
        metadata_extra={**base, "chunk_index_in_section": "1"},
    )
    after = _chunk(
        "document-1",
        1,
        ...
        ordinal=4,
        topic_path="topic-a.html",
        metadata_extra={**base, "chunk_index_in_section": "2"},
    )
    await search.upsert_chunks(
        (
            IndexedChunk("z-root", "dataset-1", root, vector),
            IndexedChunk("z-parent", "dataset-1", parent, vector),
            IndexedChunk("z-before", "dataset-1", before, vector),
            IndexedChunk("a-anchor", "dataset-1", anchor, vector),
            IndexedChunk("z-after", "dataset-1", after, vector),
        )
    ...
    result = await RetrievalService(repository, search, model).retrieve(
        RetrieveQuery("request", "dataset-1", "exact API query", 1, {}, 200)
    )
    ...
    assert [item.metadata.get("retrieval_role") for item in result.evidence] == [
        None,
        "section_neighbor",
        "section_neighbor",
        "section_parent",
    ...
    assert all(item.scores.fusion_score is None for item in result.evidence[1:])
