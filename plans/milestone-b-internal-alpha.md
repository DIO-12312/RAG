...
- [ ] B1 领域模型、状态机、digest/ID 单测通过并提交。
- [ ] B2 ports 与 Fake contract 全部通过并提交。
...
- [ ] 所有 Fake 仅存在于 `tests/fakes/`，生产 bootstrap 无测试导入。
- [ ] 未增加 HTTP、SQLite、Qdrant、数据库队列、Agent、Chat 或 SSE。
- [ ] 真实 integration/E2E 未运行项在交接和 PLAN 中明确保留。
