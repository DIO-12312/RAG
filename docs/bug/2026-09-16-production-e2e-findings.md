...
- 节流实测（滑动窗口版）：一次 2578 Chunk 的 CHM 重新索引在触发 3 次 429 后把预算收紧到 12.5 万字符/分钟，此后不再出现 429，但整篇文档明显变慢

### 10.4.1 最终复验（部署 `a16309b`：令牌桶节流 + 进度上报）

| 观察项 | 结果 |
|---|---|
| 2578 Chunk 的 `ZRDDS_C_UserManual.chm` 重新索引 | **RUNNING 期间进度 1% → 5% → 68.9% → 85.9% → 100%**，不再停在 1% |
| 限流事件 | 12 分钟内 **0 次** `embedding_request_retry`（滑动窗口版同一文档会连续触发 3–4 次并减半到 3 万字符/分钟） |
| 进度写入告警 | 0 次 `Data truncated for column 'progress'`（写入前已按 NUMERIC(7,6) 取整） |
| 摄取结果 | 全部 11 个 ZRDDS 文档 INDEXED；三份 CHM 首次成功入库 |

节流参数：`RAG_EMBEDDING_MAX_CHARS_PER_MINUTE`（默认 25 万字符/分钟）按秒补充额度、突发容量约 10 秒；
被限流后按半数收紧，每 20 次成功恢复 1.1 倍；账号配额更高时可上调该变量。
