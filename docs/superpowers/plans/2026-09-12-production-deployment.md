...
- P0-5 首次发布已完成（2026-09-13 01:19）：采用方案 A（主机 Mihomo 代理/TUN 作为拉取出口），主机直连 GHCR 从 28.7 KB/s 提升到 3.78 MB/s，三个应用镜像分别 26.1s/17.1s/2.7s 拉取完成；`release.py deploy` 在 44 秒内完成停服切换与两轮健康检查，`active.json` 推进到 `7ee76c0`/sequence 2、`previous.json` 记录 `7474c27`，回退目标明确，页面侧栏开始显示发布的短 SHA。
- P0-5 运行前提与后续：发布前必须保证 `clash-tui.service`（TUN）与 `rag-public-return-route.service` 在线，代理停掉时发布会安全失败在拉取阶段而不改动生产（排障见 `docs/deployment-release.md`）；签名/扫描仍未交付。
...
5. 用 `make production-run` 经 Earthfile 校验 `PUBLIC_MODE`、生产 env 和 Compose 配置、从当前源码构建无状态材料检查器并以禁止拉取的本地镜像校验外部材料，再移除同项目孤儿容器并启动生产服务；不得将 `config` 渲染结果重定向到文件或日志，因为其中会包含敏感路径/配置。裸 IP 阶段使用默认 `PUBLIC_MODE=ip`。
6. 域名与 ACME 条件就绪后，在维护窗口设置域名 Origin、站点地址和 Secure Cookie，执行 `make production-run PUBLIC_MODE=domain`，先验证 bootstrap、数据库、RAG 服务和 Go `/readyz`，最后验证 HTTPS。仅从浏览器访问域名；不映射或探测私网端口。
