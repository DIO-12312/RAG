...
async function resume(id:string):Promise<void>{const row=conversations.value.find(c=>c.id===id);if(!row||busy.value)return;selectAvailableDataset(row.datasetId);conversationId.value=id;try{transcript.value=await request("/conversations/"+id+"/messages?datasetId="+encodeURIComponent(selectedId.value));}catch(e){error.value=e instanceof Error?e.message:"会话加载失败";}}
watch(selectedId,(datasetId,previous)=>{if(!conversationId.value||busy.value||!datasetId||datasetId===previous)return;transcript.value=[];void request<{role:string;content:string;citations:Citation[]}[]>("/conversations/"+conversationId.value+"/messages?datasetId="+encodeURIComponent(datasetId)).then(messages=>{if(selectedId.value===datasetId)transcript.value=messages;}).catch(e=>{error.value=e instanceof Error?e.message:"会话加载失败";});});
