...
import {getSettings,updateChatSettings,updateEmbeddingSettings,updateRerankSettings} from "@/api/settings";
...
onMounted(async()=>{try{settings.value=await getSettings();}catch(e){error.value=e instanceof Error?e.message:"配置加载失败";}});
async function save(kind:"chat"|"embedding"|"rerank"):Promise<void>{if(!settings.value||busy.value)return;busy.value=true;error.value="";notice.value="";try{let result:SettingsResponse;if(kind==="chat")result=await updateChatSettings({...settings.value.chat,apiKey:keys.value.chat||undefined});else if(kind==="embedding")result=await updateEmbeddingSettings({...settings.value.embedding,apiKey:keys.value.embedding||undefined});else result=await updateRerankSettings({...settings.value.rerank,apiKey:keys.value.rerank||undefined});settings.value=result;keys.value[kind]="";notice.value="已保存";}catch(e){error.value=e instanceof Error?e.message:"保存失败";}finally{busy.value=false;}}
...
<template>
  <main class="settings-page">
    ...
    >
      ...
      <article class="settings-card">
        <div class="settings-card-heading">
          <span class="icon-tile icon-tile--peach"><AppIcon name="settings" /></span><div><h2>Rerank 模型</h2><span class="eyebrow">COMING NEXT</span></div>
        </div><p>配置可以保存，当前检索链路暂不启用用户级 Rerank。</p><form @submit.prevent="save('rerank')">
          <label>Base URL<input
