...
func (s *Server) chat(c *gin.Context) {
	...
	h := agent.Harness{Model: agent.OpenAI{BaseURL: base, Key: apiKey, Name: name, Timeout: time.Duration(timeout) * time.Second, Thinking: thinking, Client: modelClient}, Tool: s.RAG, MaxRounds: 6, TopK: int(top), Streaming: true}
